 ...
 <section class="cstyle_ctop">
	<div class="row">
		<div class="col-lg-12 cstyle_cbottom">
            ...
            <div class="float-left"><i class="fa fa-users"></i> Deactive Users </div>
            <div class="float-right">
              <a href="users.php" class="btn btn-info btn-lg">Active Users </a>
            </div>
			  </div>
			  <div class="card-body">
          ...
          <table id="userstable" class="table table-bordered">
            ...
            <tbody>
            <?php 
             if (method_exists($admin, 'getDeactiveUsers')) {
               $udata = $admin->getDeactiveUsers();
               if ($udata) {
                 $i=0;
            ...
            ?>
              ...
              <td class="text-center"><button type="button" class="btn btn-info btn-lg onclickusers" data-usersid="<?php echo $users_id; ?>" data-toggle="modal" data-target="#users_modal"><i class="fa fa-eye"></i> View </button>
                <button type="button" class="btn btn-primary btn-lg onclickactive" data-useraid="<?php echo $users_id; ?>">Active</button>
              </td>
            </tr>
            <?php } } } ?>
            </tbody>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('.onclickactive').on('click', function(){
      var useraid = $(this).data('useraid');
       $.ajax({url:"../ajax/user_active.php?useraid="+useraid+"&useractive=48",success:function(useradata){
        var strFirstTwo = useradata.substring(0,2);
          if (strFirstTwo == 68) {
            var successdata = useradata.substring(2);
            $('#output_message').html(successdata);                 
            setTimeout(function(){
             location.reload();
            },2000);
          }else{
            $('#output_message').html(useradata);
          }
         }});
    });
